...
const handleBulkStageChange = async () => {
  ...
  // Update lead stage
  const handleChangeStage = async (leadId: string, stageId: string,reason:string) => {
    try {
      setChangingStage(true);
      const endpoint = ApiConfig.changeStageLead(leadId);
      const response = await patchTokenDataHandler(endpoint, { stageId,reason }, true);
  ...
  const handleStageSubmit = async (leadId: string, stageId: string,reason:string) => {
    await handleChangeStage(leadId, stageId,reason);
  };
