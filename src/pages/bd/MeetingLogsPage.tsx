  ...
  const handleAddFeedback = async () => {
    try {
      ...
      if (!feedbackForm.meetingId || !feedbackForm.feedback.trim()) {
        toast({ title: 'Error', description: 'Feedback is required', variant: 'destructive' });
        return;
      }
      await postDataHandlerWithToken('addMettingFeedback', {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-slate-500 mt-1">Track and manage all meeting interactions (BDA Panel)</p>
          </div>
          <div className="flex items-center gap-2">
            {/* Export button */}
            <Select onValueChange={(value) => exportToCSV(value === 'all')}>
              <SelectTrigger className="w-full sm:w-[180px] bg-white border-slate-200 shadow-sm rounded-xl">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-slate-50 border-b border-slate-100">
                    ...
                    <TableHead className="text-xs font-semibold text-slate-500 uppercase">Feedback</TableHead>
                  ...
                  {logs.map((log) => (
                    ...
                    >
                      ...
                      className = "flex items-center justify-center">
                        <button 
                        className="h-8 w-8 p-0 rounded-lg hover:bg-slate-100 flex items-center justify-center"
                        onClick={() => {
                        setSelectedLog(log);
                        setDetailsModalOpen(true);
                      }}><MessageCircle className="w-4 h-4 text-slate-400"/></button>
                      </TableCell>
                      <TableCell>
                        <Button
                          variant="ghost"
          ...
          <DialogContent className="rounded-2xl max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            {selectedLog && (
              <div className="space-y-5">
                ...
                {!selectedLog.feedbacks?.length && (
                  <div className="flex justify-end">
                    <Button 
                    variant="outline" 
                    size="sm" onClick={(e) => { 
                      e.stopPropagation(); handleOpenAddFeedback(selectedLog); }} className="rounded-lg">
                      <MessageCircle className="w-4 h-4 mr-2" /> Add Feedback
                    </Button>
                  </div>
                )}
              </div>
        ...
        <Dialog open={feedbackModalOpen} onOpenChange={setFeedbackModalOpen}>
          <DialogContent className="rounded-2xl max-w-md">
            <DialogHeader>
              <DialogTitle className="text-xl">Meeting Feedback</DialogTitle>
              <DialogDescription>Recorded feedback</DialogDescription>
            </DialogHeader>
            <div className="py-4">
              {selectedFeedback ? (
                <div className="bg-slate-50 p-4 rounded-xl text-sm whitespace-pre-wrap">{selectedFeedback}</div>
              ) : (
                <div className="text-center py-8 text-slate-500">No feedback provided</div>
              )}
            </div>
            <DialogFooter><Button onClick={() => setFeedbackModalOpen(false)} className="rounded-xl">Close</Button></DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={addFeedbackModalOpen} onOpenChange={setAddFeedbackModalOpen}>
          <DialogContent className="rounded-2xl max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>Enter feedback for the meeting</DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setAddFeedbackModalOpen(false)} className="rounded-xl">Cancel</Button>
              <Button onClick={handleAddFeedback} disabled={addingFeedback || !feedbackForm.feedback.trim()} className="bg-orange-600 hover:bg-orange-700 rounded-xl">
                {addingFeedback ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Plus className="w-4 h-4 mr-2" />}
                Submit Feedback
              </Button>
            </DialogFooter>
          </DialogContent>
