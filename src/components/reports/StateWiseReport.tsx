...
export function StateWiseReport() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (campaignSearch.trim()) params.campaign = campaignSearch.trim();
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `campaign_state_report_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  // Grand totals
  const totalCampaigns = data?.data?.length || 0;
  const totalLeads = data?.data?.reduce((sum, c) => sum + c.totalLeads, 0) || 0;
  const totalAdmissions = data?.data?.reduce((sum, c) => sum + c.totalAdmissionDone, 0) || 0;
  const totalRevenue = data?.data?.reduce((sum, c) => sum + c.totalRevenue, 0) || 0;

  // Get unique states across all campaigns
  const uniqueStates = new Set<string>();
  data?.data?.forEach(campaign => {
    campaign.states.forEach(state => uniqueStates.add(state.state));
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-base font-semibold text-slate-800">Campaign State Report</h3>
          <p className="text-sm text-slate-500">Lead distribution across campaigns and states</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          {/* Summary Cards - New design with gradient accents */}
            ...
            <Card className="p-4 bg-white border-0 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-800 mt-1">{totalCampaigns}</p>
                </div>
          ...
          {/* Campaign List - New card-based design */}
          <div className="space-y-3">
            {data.data.map((campaign, index) => {
              const isExpanded = expandedCampaigns.has(campaign.campaignName);
              const topStates = campaign.states.slice(0, 3);
              const remainingStates = campaign.states.length - 3;
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-500 mt-0.5 flex-wrap">
                          <span className="flex items-center gap-1">
                            ...
                            {campaign.states.length} states
                          </span>
                          <span className="w-1 h-1 rounded-full bg-slate-300" />
                          <span>{campaign.totalLeads} leads</span>
                          <span className="w-1 h-1 rounded-full bg-slate-300" />
                          <span>{campaign.totalAdmissionDone} admissions</span>
                          <span className="w-1 h-1 rounded-full bg-slate-300" />
                          <span className="font-medium text-emerald-600">₹{campaign.totalRevenue.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
